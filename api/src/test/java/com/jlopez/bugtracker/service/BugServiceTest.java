...
public class BugServiceTest {
  ...
  public void it_returns_an_empty_optional_when_updating_a_non_existent_bug() {
    BugUpdateRequestPayload updateRequest =
        BugUpdateRequestPayload.builder().id(200L).name("Testing Update").description("Updated description").build();

    Optional<BugPayload> maybeUpdatedBug = bugService.update(updateRequest);
  ...
  public void it_returns_an_updated_bug_payload_when_updating_a_non_existent_bug() {
    ...
    BugUpdateRequestPayload updateRequest =
        BugUpdateRequestPayload.builder().id(1L).name("Testing Update in Test").description("Updated description").status(requestedStatus)
            .build();
  ...
  public void it_returns_a_bug_payload_when_creating_a_new_bug() {
    StatusPayload requestedStatus = StatusPayload.builder().id(1L).name("Pending").build();
    BugCreationRequestPayload creationRequest =
        BugCreationRequestPayload.builder().name("Testing Create").description("Created description").status(requestedStatus).build();

    BugPayload createdBug = bugService.create(creationRequest);

    Assertions.assertNotNull(createdBug);
