...
public class BugControllerTest extends WebMvcTestConfiguration {
  ...
  public void it_returns_ok_when_findById_is_successful() throws Exception {
    BugPayload bugPayload =
        BugPayload.builder().id(1L).name("Test").description("Test").createdAt(LocalDateTime.now()).updatedAt(LocalDateTime.now()).build();
  ...
  public void it_returns_ok_when_updating_an_existing_bug() throws Exception {
    when(bugService.update(any())).thenReturn(Optional.of(BugPayload.builder().build()));

    BugUpdateRequestPayload updatePayload = BugUpdateRequestPayload.builder().id(1L).name("Test update").description("Test Update")
        .status(StatusPayload.builder().id(1L).name("Pending").build()).build();
    ...
    verify(bugService, times(1)).update(any());
  }

  @Test
  public void it_returns_not_found_when_updating_a_non_existing_bug() throws Exception {
    when(bugService.update(any())).thenReturn(null);

    BugUpdateRequestPayload updatePayload = BugUpdateRequestPayload.builder().id(200L).name("Test update").description("Test Update")
        .status(StatusPayload.builder().id(1L).name("Pending").build()).build();

    mockMvc.perform(put("/bug/200").contentType(MediaType.APPLICATION_JSON).content(asJsonString(updatePayload)))
  ...
  public void it_returns_bad_request_when_updating_with_no_name() throws Exception {
    BugUpdateRequestPayload updatePayload =
        BugUpdateRequestPayload.builder().id(1L).description("Test Update").status(StatusPayload.builder().id(1L).name("Pending").build())
            .build();

    mockMvc.perform(put("/bug/1").contentType(MediaType.APPLICATION_JSON).content(asJsonString(updatePayload)))
        .andExpect(status().isBadRequest());

    verify(bugService, times(0)).update(any());
  }
  ...
  public void it_returns_ok_when_new_bug_is_created() throws Exception {
    when(bugService.create(any())).thenReturn(BugPayload.builder().build());
    ...
    mockMvc.perform(post("/bug").contentType(MediaType.APPLICATION_JSON).content(asJsonString(updatePayload)))
